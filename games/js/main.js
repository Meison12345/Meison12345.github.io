let app = new Vue({
    ...
    methods: {
        ...
        endGame() {
            ...
            // Зерг
            if (this.score.zerg > this.score.protoss && this.score.zerg > this.score.terran && this
                .score.primal < 8 && Math.abs(this.score.protoss - this.score.zerg) > 3) {
    ...
    computed: {
        totalScore() {
            let score = 0
            for (let k in this.totalGame) {
                // console.log(results[k]);
                score += (this.totalGame[k] * results[k].points)
            }
            return score
        },
        openRaces() {
        ...
        favoriteRace() {
            let max = 'zerg'
            for (let k in this.totalGame) {
                if (this.totalGame[k] > this.totalGame[max]) {
                    max = k;
                }
            }
